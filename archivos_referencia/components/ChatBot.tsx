...
const ChatBot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const responseFullText = await sendMessageToGemini(userText);
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-4 right-4 md:bottom-8 md:right-8 z-50 p-3 md:p-4 rounded-full shadow-2xl transition-all duration-300 transform hover:scale-105 active:scale-95 flex items-center justify-center ${
          isOpen ? 'bg-coffee-900 rotate-90' : 'bg-gradient-to-r from-gold-500 to-gold-600 hover:from-gold-400 hover:to-gold-500'
        } text-white`}
        ...
        <div className="fixed bottom-20 right-4 md:bottom-28 md:right-8 w-[calc(100vw-2rem)] md:w-[360px] h-[600px] max-h-[80vh] bg-white rounded-2xl shadow-2xl flex flex-col z-50 overflow-hidden border border-coffee-100 animate-fade-in-up font-sans">
          ...
          <div className="flex-1 overflow-y-auto p-4 bg-coffee-50 space-y-3">
            ...
            {messages.map((msg) => (
              ...
              >
                <div
                  className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-[15px] shadow-sm relative leading-snug ${
                    msg.role === 'user'
